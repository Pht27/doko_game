Research, implement, and close out a todo item on a feature branch.
...
- If on `develop`: create a new feature branch from it (`git checkout -b feature/<short-name>`) named after the todo.
- If already on a feature branch: keep working on it, do not switch.
...
### 7. **Update release notes** — add what was implemented to the current latest version block at the top of `RELEASENOTES.md`:
   ...
   - Use subsections `### Neu`, `### Behoben`, or `### Verbessert` as appropriate (they may preexist in that version block)
...
### 10. **Commit** the changes with fitting commit messages that summarize what was done in sensible chunks.

### 11. **Merge into develop** — when the user explicitly asks to merge:
- Squash-merge the feature branch into `develop`:
  ```sh
  git checkout develop
  git merge --squash feature/<name>
  git commit -m "<summary of the feature>"
