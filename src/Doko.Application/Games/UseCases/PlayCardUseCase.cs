    ...
    IGameScorer scorer) : IPlayCardUseCase
{
    ...
    public async Task<GameActionResult<PlayCardResult>> ExecuteAsync(PlayCardCommand command, CancellationToken ct = default)
    {
        ...
        if (state is null)                            return Fail(GameError.GameNotFound);
        if (state.Phase != GamePhase.Playing)         return Fail(GameError.InvalidPhase);
        if (state.CurrentTurn != command.Player)      return Fail(GameError.NotYourTurn);
        ...
        if (card is null)                             return Fail(GameError.IllegalCard);

        BeginTrickIfNeeded(state);

        if (!CardPlayValidator.CanPlay(card, playerState.Hand, state.CurrentTrick!, state.TrumpEvaluator))
            return Fail(GameError.IllegalCard);

        var sonderkarteError = ApplySonderkarten(state, card, command, out var sonderkarteEvents);
        if (sonderkarteError.HasValue) return Fail(sonderkarteError.Value);
    ...
    {
        if (state.CurrentTrick is not null) return;
    ...
    private static GameError? ApplySonderkarten(
        GameState state, Card card, PlayCardCommand command,
        out List<IDomainEvent> events)
    {
        ...
        foreach (var sonderkarte in eligible.Where(s => activateSet.Contains(s.Type)))
        {
            ...
            foreach (var mod in mods)
                state.Apply(mod);
            events.Add(new SonderkarteTriggeredEvent(state.Id, command.Player, sonderkarte.Type, mods));
        }

        var genscherError = ApplyGenscherIfNeeded(state, command, activateSet);
        if (genscherError.HasValue) return genscherError;
    ...
    private static GameError? ApplyGenscherIfNeeded(
        GameState state, PlayCardCommand command, HashSet<SonderkarteType> activateSet)
    {
        bool genscherActivated = activateSet.Contains(SonderkarteType.Genscherdamen)
                              || activateSet.Contains(SonderkarteType.Gegengenscherdamen);
        if (!genscherActivated) return null;

        if (command.GenscherPartner is null)                               return GameError.GenscherPartnerRequired;
        if (command.GenscherPartner == command.Player)                     return GameError.GenscherPartnerInvalid;
        if (state.Players.All(p => p.Id != command.GenscherPartner))      return GameError.GenscherPartnerInvalid;

        state.Apply(new SetPartyResolverModification(
            new GenscherPartyResolver(command.Player, command.GenscherPartner.Value)));
        return null;
    }

    private static void PlayCardIntoTrick(GameState state, PlayerId player, PlayerState playerState, Card card)
    {
    ...
    private static List<IDomainEvent> BuildEvents(
        GameState state, PlayerId player, Card card, List<IDomainEvent> sonderkarteEvents)
        => [.. sonderkarteEvents, new CardPlayedEvent(state.Id, player, card, state.CompletedTricks.Count)];

    private async Task<GameActionResult<PlayCardResult>> AdvanceTurnAsync(
        GameState state, PlayerId player, List<IDomainEvent> events, CancellationToken ct)
    {
    ...
    private async Task<GameActionResult<PlayCardResult>> CompleteTrickAsync(
        GameState state, List<IDomainEvent> events, CancellationToken ct)
    {
        ...
        events.Add(new TrickCompletedEvent(state.Id, state.CompletedTricks.Count - 1, winner, result));
    ...
    private async Task SaveAndPublishAsync(GameState state, List<IDomainEvent> events, CancellationToken ct)
    {
