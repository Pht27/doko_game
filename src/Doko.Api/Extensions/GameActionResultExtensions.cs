...
public static class GameActionResultExtensions
{
    ...
    private static IActionResult MapError(GameError error) =>
        error switch
        {
            ...
            GameError.InvalidPhase => new BadRequestObjectResult(
                new ErrorResponse("invalid_phase")
            ),
            GameError.IllegalCard => new BadRequestObjectResult(new ErrorResponse("illegal_card")),
            GameError.AnnouncementNotAllowed => new BadRequestObjectResult(
                new ErrorResponse("announcement_not_allowed")
            ),
            GameError.ReservationNotEligible => new BadRequestObjectResult(
                new ErrorResponse("reservation_not_eligible")
            ),
            GameError.SonderkarteNotEligible => new BadRequestObjectResult(
                new ErrorResponse("sonderkarte_not_eligible")
            ),
            GameError.AlreadyDeclared => new BadRequestObjectResult(
                new ErrorResponse("already_declared")
            ),
            GameError.GenscherPartnerRequired => new BadRequestObjectResult(
                new ErrorResponse("genscher_partner_required")
            ),
            GameError.GenscherPartnerInvalid => new BadRequestObjectResult(
                new ErrorResponse("genscher_partner_invalid")
            ),
