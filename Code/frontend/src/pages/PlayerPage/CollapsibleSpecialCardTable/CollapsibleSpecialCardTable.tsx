import type { PlayerSpecialCardStat } from '@/types/analog';
...
function toGroups(rows: PlayerSpecialCardStat[]): OccurrenceGroup[] {
  const map = new Map<number, OccurrenceGroup>();
  for (const row of rows) {
    if (!map.has(row.specialCardId))
      map.set(row.specialCardId, { id: row.specialCardId, name: row.specialCardName, re: null, kontra: null });
...
export function CollapsibleSpecialCardTable({ rows }: { rows: PlayerSpecialCardStat[] }) {
