import type { PlayerExtraPointStat } from '@/types/analog';
...
function toGroups(rows: PlayerExtraPointStat[]): OccurrenceGroup[] {
  const map = new Map<number, OccurrenceGroup>();
  for (const row of rows) {
    if (!map.has(row.extraPointId))
      map.set(row.extraPointId, { id: row.extraPointId, name: row.extraPointName, re: null, kontra: null });
...
export function CollapsibleExtraPointTable({ rows }: { rows: PlayerExtraPointStat[] }) {
